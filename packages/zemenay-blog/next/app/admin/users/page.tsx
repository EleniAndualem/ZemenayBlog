                  ...
                  <tr key={user.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                    ...
                    <td className="px-6 py-4">
                      {editingUser === user.id ? (
                        <select
                          ...
                          onChange={(e) => setEditData((prev) => ({ ...prev, roleId: Number.parseInt(e.target.value) }))}
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end space-x-2">
                        {editingUser === user.id ? (
                          <>
                            <button onClick={() => updateUser(user.id, editData)} className="p-2 text-green-600 hover:text-green-700 transition-colors" title="Save changes">
                              <Shield className="h-4 w-4" />
                            </button>
                            <button onClick={() => setEditingUser(null)} className="p-2 text-gray-400 hover:text-gray-600 transition-colors" title="Cancel">×</button>
                          </>
                        ) : (
                          <>
                            <button onClick={() => { setEditingUser(user.id); setEditData({ roleId: user.role.id, darkMode: user.darkMode }) }} className="p-2 text-gray-400 hover:text-blue-600 transition-colors" title="Edit user">
                              <Edit className="h-4 w-4" />
                            </button>
                            {user.id !== currentUser?.id && user.role.name !== "superadmin" && (
                              <button onClick={() => deleteUser(user.id)} className="p-2 text-gray-400 hover:text-red-600 transition-colors" title="Delete user">
